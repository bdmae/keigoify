...
export function usePrompt() {
  ...
  const buildPrompt = useCallback((input: string, formality: string): string => {
    return `
You are a professional translator specializing in Japanese language politeness levels. Your task is to translate a given text into Japanese using one of three politeness levels: casual, formal, or keigo (honorific). Please follow these instructions:
